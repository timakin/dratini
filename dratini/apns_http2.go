...
func NewTransportHttp2(cert tls.Certificate) (*http.Transport, error) {
	...
	transport := &http.Transport{
		...
		MaxIdleConnsPerHost: ConfDratini.Ios.KeepAliveConns,
		Dial: (&net.Dialer{
			Timeout:   time.Duration(ConfDratini.Ios.Timeout) * time.Second,
			KeepAlive: time.Duration(keepAliveInterval(ConfDratini.Ios.KeepAliveTimeout)) * time.Second,
		}).Dial,
		IdleConnTimeout: time.Duration(ConfDratini.Ios.KeepAliveTimeout) * time.Second,
	}
