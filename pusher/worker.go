...
func pushNotificationWorker() {
	...
	for {
		...
		case PlatFormIos:
			...
			retryMax = ConfDratini.Ios.RetryMax
		case PlatFormAndroid:
			pusher = pushNotificationAndroid
			retryMax = ConfDratini.Android.RetryMax
		default:
			LogError.Warn(fmt.Sprintf("invalid platform: %d", notification.Platform))
			continue
		}

		if atomic.LoadInt64(&ConfDratini.Core.PusherMax) <= 0 {
			pushSync(pusher, notification, retryMax)
			continue
		}

		if atomic.LoadInt64(&pusherCount) < atomic.LoadInt64(&ConfDratini.Core.PusherMax) {
